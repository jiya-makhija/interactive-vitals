...
yAxisLabel.text("Average Vital Value");
...
Promise.all([
  d3.csv("data/long_surgery_1300_vitals.csv", d3.autoType),
  ...
  function updateChart() {
    ...
    yAxisLabel.text(selectedVital === "stability_index" ? "Stability Index" : "Average Vital Value");
    ...
    const summary = nested.map(([key, values]) => {
      ...
      const binned = d3.groups(values, d => Math.round(d.norm_time / binSize) * binSize)
        .map(([t, pts]) => {
          const v = pts.map(p => selectedVital === "stability_index" ? p.stability_index : p.value);
